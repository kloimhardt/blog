...
The following code example, when executed, shows on the computer screen the simple graph of a straight line. Being a publicly available tutorial [1], it introduces a certain software which is maintained by the European center for particle research (CERN), namely the library called ROOT. The example is going to be referred to over the course of this article, so at first we reproduce its nine lines of Python code.
...
This Python code is going to be translated and compared to Clojure [2], a programming language currently mainly used for commercial web applications [3]. Clojure is a dialect of the Lisp family of languages.

Despite both languages being multi-paradigm in principle, Python and Clojure respectively emphasise in their usage two different paradigms in software design: "object oriented" and "functional". The ultimate purpose of the translation is to further advance and demonstrate the usage of functional programming in the natural sciences.

## Header

While the CERN tutorial script to plot a graph is written in Python, the ROOT library is programmed in the language C++. The necessary interoperation between the scripting language and the library is done via PyROOT, an “automatic, run-time, Python-C++ bindings generator” [4].
...
With this beginning statement, the very flexible and feature rich PyROOT bridge software starts to operate behind the scenes as the subseqent Python code is interpreted and executed.

## Header translation

We now turn to the first main goal of this article, the translation of the Python code into the Lisp language. As opposed to the Python case, we do not use a runtime bridge software to access ROOT. Instead, Lisp code is transpiled to C++ ahead of time, that is before running it. For transpilation we use Ferret, a free software Lisp implementation supporting a subset of Clojure designed to generate executable binaries for "systems with as little as 2KB of RAM" [5].
...
By calling `ROO/T` with `:my-hint`, any graph can be dotted point by point, the value of the respective argument being the letter "P".
