...
During the course of this article we are continuously going to refer to a publicly available tutorial [1] which introduces software maintained by the European center for particle research (CERN), namely a library called ROOT. The example's nine lines of Python code, which upon execution call into ROOT to show on the computer screen the simple graph of a straight line, are reproduced here.
