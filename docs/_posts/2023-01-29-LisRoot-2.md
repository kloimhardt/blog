...
The following code example, when executed, shows on the computer screen the simple graph of a straight line. Being a publicly available tutorial [1], it introduces a certain software which is maintained by the European center for particle research (CERN), namely the library called ROOT. This example is going to be referred to over the course of this article, so at first we reproduce its nine lines of Python code.
...
Despite both languages being multi-paradigm in principle, Python and Clojure respectively represent two different paradigms in software design: "object oriented" and "functional". The ultimate purpose of our translation is to further advance and demonstrate the usage of functional programming in the natural sciences.

## Header

While the CERN tutorial to plot a graph is written in Python, the ROOT library is programmed in the language C++. The necessary interoperation between the scripting language and the library is done via PyROOT, an “automatic, run-time, Python-C++ bindings generator” [4].
