...
In communication to the wider Clojure community, this requirement memory=disk can be labeled as the Figwheel-reloaded approach. In talking to Python people it does not need a name, memory=disk is the normal situation. In the same way that you do not ask a fish "how is the water today", you do not ask a Pythonist "how is your disk=memory today". It is exactly for this reason, namely to be relatable to Pythonists, that we provide the Noj-CLI with live-reload as default.
...
Clay has the advantage over an IDE that the browser window offers lots of space for additional linting information, e.g. statically inferred types. Also a sophisticated debugger like Flowstorm could be part of the Clay experience.
...
I argue that SciCloj's core task is to refer to already existing videos and blogs of the wider Clojure community. Also, encourage the wider Clojure community to watch Notepad++ videos and understand the presented Noj examples. Then those existing Noj examples can be stones in the mosaic that shows Clojure superpowers. SciCloj should leave this space open, generously leave this kind of fun to acquaintances from the wider Clojure community.

I further argue that SciCloj's core task is to show a concept that has been neglected up to now: the REPL prompt. The prompt is also well known to Python users and has nothing to do with an IDE.
